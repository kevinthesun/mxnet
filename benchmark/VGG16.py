...
parser.add_argument('--batch_size', type=int, default='32',
                        help='Batch size')
parser.add_argument('--image_shape', type=str, default='3,299,299',
...
if __name__ == '__main__':
    ...
    data_shape = tuple([batch_size] + [int(l) for l in args.image_shape.split(',')])
    ...
    for i in range(epoch):
        ...
        for batch in train_data:
            data_split = gluon.utils.split_and_load(batch.data[0], ctx_list=ctx, batch_axis=0)
            label_split = gluon.utils.split_and_load(batch.label[0], ctx_list=ctx, batch_axis=0)
            outputs = []
            with ag.record():
                for data, label in zip(data_split, label_split):
                    out = model(data)
                    loss = softmax_loss(out, label)
                    ag.backward([loss])
                    outputs.append(out)
            metric.update(label_split, outputs)
            trainer.step(batch.data[0].shape[0])
        name, acc  =metric.get()
        metric.reset()
        print('training acc at epoch %d: %s=%f' % (i, name, acc))
    total_time = time.time() - start_time
    print("Training time: %s" % (str(total_time)))
    print("Training time per epoch: %s" % (str(total_time / epoch)))
    print("Number of images process per second: %s" % (str(batch_size * max_iter * epoch / total_time)))
